  ...
  const handleDelete = () => {
    swal({
      ...
      .then(async (willDelete) => {
        if (willDelete) {
          try {
            const res = await armeniaClient.thingToSee.deleteByID(thing.id)
            toast.success('Thing to See deleted successfully!')
            router.push({
        ...
        <div className="p-3">
          ...
          <div className="flex justify-end items-end">
            <div className="flex items-center gap-3">
              <Link href={`/admin/thing-to-see/update/${thing.id}`}>
